...
public class ChunkCubeXCube : MonoBehaviour
{
    ...
    int indexFromCord(int x, int y, int z)
    {
        return (x + y * chunkSize.x + z * chunkSize.x * chunkSize.y);
    } 

    void GetVertices(float[] cubeValues, Vector3 position)
    {
        ComputeBuffer triBuffer = new ComputeBuffer(numMaxTri, sizeof(float) * 3 * 3, ComputeBufferType.Append);
        ComputeBuffer triCountBuffer = new ComputeBuffer(1, sizeof(int), ComputeBufferType.Raw);
        ComputeBuffer pointBuffer = new ComputeBuffer(cubeValues.Length, sizeof(float));

        pointBuffer.SetData(cubeValues);
        ...
        for (int i = 0; i < numTris; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                ...
                vertices.Add(tris[i][j] + position);
            }
    ...
    public void GenerateMesh()
    {
        ...
        int groupSize = Mathf.CeilToInt(chunkSize.x / 2);
        ...
        shader.SetBuffer(kernel, "triangles", triBuffer);
        shader.Dispatch(0, groupSize, groupSize, groupSize);
        ...
        //        {
        //            //Prendi valori del cubo
        //            float[] cube = new float[8]
        //            {
        //                voxelValues[indexFromCord(x, y, z)],
        //                voxelValues[indexFromCord(x, y+1, z)],
        //                voxelValues[indexFromCord(x+1, y+1, z)],
        //                voxelValues[indexFromCord(x+1, y, z)],
        //                voxelValues[indexFromCord(x, y, z+1)],
        //                voxelValues[indexFromCord(x, y+1, z+1)],
        //                voxelValues[indexFromCord(x+1, y+1, z+1)],
        //                voxelValues[indexFromCord(x+1, y, z+1)]
        //            };

        //            //Per ogni cubo lancia una shader e aggiungi alle liste
        //            GetVertices(cube, new Vector3(x, y, z));
        //        }
        //    }
        //}
